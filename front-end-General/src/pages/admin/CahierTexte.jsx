...
import "./AdminDepartments.css"; // Reuse existing styles or create new

const AdminCahierTexte = () => {
    ...
    const openModal = (note = null) => {
        if (note) {
            ...
            setFormData({
                titre: "",
                contenu: "",
                seanceId: "", // Should be selected from Emploi du Temps ideally
                enseignantId: "",
                objectifsPedagogiques: "",
                activitesRealisees: "",
                travailDemande: "",
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...formData,
                seanceId: parseInt(formData.seanceId), // Ensure number
    ...
    const handleValidate = async (id) => {
        try {
            await apiRequest(API_ENDPOINTS.CAHIER_TEXTE.VALIDATE(id), { method: "PATCH" }); // or POST
            fetchNotes();
        } catch (err) {
    ...
    return (
        <div className="admin-departments-container">
            <div className="header-actions">
                <h2>Cahier de Texte (Centralisé)</h2>
...
export default AdminCahierTexte;
